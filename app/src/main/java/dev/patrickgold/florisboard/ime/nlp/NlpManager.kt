...
class NlpManager(context: Context) {
    ...
    private fun assembleCandidates() {
        val candidates = buildList {
            // FIX: We create a flag to see if we have manual suggestions (Vibe/Sentiment) waiting
            val hasManualSuggestions = suggestions.value?.isNotEmpty() == true

            // FIX: We allow processing if suggestions are enabled OR if we have manual ones
            if (prefs.smartbar.enabled.get() && (prefs.suggestion.enabled.get() || hasManualSuggestions)) {
                ...
                suggestions.value?.let { suggestionList ->
                    suggestionList.forEachIndexed { n, word ->
                        // FIX: Ensure auto-insert is false for manual Vibe suggestions to prevent accidental typing
