 ...
 */
/*
    to create paste this in terminal of android studio
    ./gradlew assembleDebug
    then paste this to get location on finder
...
//for running ml models on diff threads
...
class FlorisImeService : LifecycleInputMethodService(), EditorInstance.WordHistoryChangedListener {
    companion object {
        // NEW: Helper to call emojify from UI
        ...
        fun performEmojifyAction() {
            FlorisImeServiceReference.get()?.executeEmojify()
        }
        fun showUi() {
            val ims = FlorisImeServiceReference.get() ?: return
    ...
    private lateinit var emojiPredictor: EmojiPredictor


    private lateinit var sentimentPredictor: SentimentPredictor

    private lateinit var emojifier: Emojifier

    //for threads
    private var predictionJob: Job? = null
    private val predictionScope = CoroutineScope(Dispatchers.Main + SupervisorJob())
    private val predictionMutex = Mutex() //  Thread Locking mechanism
    ...
    override fun onCreate() {
        emojiPredictor = EmojiPredictor(this)
        sentimentPredictor = SentimentPredictor(this) // new
        emojifier = Emojifier(this) // <--- NEW
        super.onCreate()
    ...
    override fun onDestroy() {
        super.onDestroy()
        predictionScope.cancel()// shutdown downs  exiting threads when keyboard is exited
    ...
    override fun onUpdateSelection(
        ...
        // Cancel the previous UI update flag, but respect the native C++ execution
        ...
        // PREDICITON & SENTIMENT INTEGRATION
        val ic = currentInputConnection
        if (ic != null) {
            val textBefore = ic.getTextBeforeCursor(200, 0)?.toString() ?: ""

            // gets executed when a space is it end
            if (textBefore.endsWith(" ")) {
                val cleanText = textBefore.trim()

                // an empty or single character returns immediatley
                if (cleanText.length < 2) return

                //the models are started in background thread
                predictionJob = predictionScope.launch(Dispatchers.Default) {
                    val finalSuggestions = ArrayList<String>()

                    // If it has a lock only it gets executed(only one thread can have lock at atime)
                    predictionMutex.withLock {

                        // If typing is detected gets aborted
                        if (!isActive) return@launch

                        // A: Prediction emoji model run
                        try {
                            val predictedEmojis = emojiPredictor.predict(cleanText, 3)
                            finalSuggestions.addAll(predictedEmojis)
                        } catch (e: Exception) {
                            Log.e("FlorisDebug", " VibePredictor Failed: ${e.message}")
                        }

                        // B: Sentiment detection model run
                        try {
                            val sentimentEmojis = sentimentPredictor.predictEmojis(cleanText, 3)
                            // only gets added into final emojis if it is not already predicted by prediction model
                            finalSuggestions.add("| ")
                            for (emoji in sentimentEmojis) {
                                if (!finalSuggestions.contains(emoji)) {
                                    finalSuggestions.add(emoji)
                                }
                            }
                        } catch (e: Exception) {
                            Log.e("FlorisDebug", " SentimentPredictor Failed: ${e.message}")
                        }
                    } // fun releases the lock it has

                    // If typing is detected gets aborted
                    if (!isActive) return@launch

                    // C: final emojis are suggested in smartbar via main thread
                    if (finalSuggestions.isNotEmpty()) {
                        val cappedSuggestions = finalSuggestions.take(6)

                        withContext(Dispatchers.Main) { // uses Main thread
                            try {
                                nlpManager.suggestDirectly(cappedSuggestions)
                                Log.d("FlorisDebug", " Force sent ${cappedSuggestions.size} suggestions to Smartbar!")
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
                        }
                    } else {
                        Log.d("FlorisDebug", " No suggestions found from either model.")
                    }
    ...
    fun executeEmojify() {
        ...
        // 1. Get text before cursor (limit to 1000 chars to prevent lag)
        val textBefore = ic.getTextBeforeCursor(1000, 0)?.toString() ?: ""
        if (textBefore.isEmpty()) return

        // 2. Run your Emojifier logic
        // Use 0.4f density so it doesn't spam too many emojis
        val newText = emojifier.emojify(textBefore, 0.4f)

        // 3. Replace the text safely
        ic.beginBatchEdit()
